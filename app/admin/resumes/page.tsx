...
export default async function AdminResumeAnalyticsPage() {
  ...
  // 1. Fetch resume, matching, and career analysis database aggregates
  const [
    totalResumes,
    parsedResumesCount,
    avgParseTimeResult,
  ...
  // 2. Resume parsing conversions
  const avgParseTimeMs = avgParseTimeResult._avg.processingTimeMs || 0;
  const avgParseTimeSec = Math.round((avgParseTimeMs / 1000) * 10) / 10;
  ...
  // Total System AI Metrics
  const combinedTokens = totalResumeTokens + totalCareerTokens;
  const combinedCost = Math.round((estimatedResumeCost + totalCareerCost) * 10000) / 10000;
  const avgSystemLatencySec = Math.round(((avgParseTimeSec + avgCareerLatencySec) / 2) * 10) / 10;

  // 4. Most Common Skills Extraction
  ...
  // 5. Most Missing Skills / Tech gaps Extraction
  ...
  // 6. Top Recommended Skills (from Roadmaps count)
  ...
  const cards = [
    { label: 'Avg Career Score', value: `${avgCareerScore}/100`, desc: `${totalCareerAnalyses} Profiles analyzed`, icon: Award, color: 'text-primary border-primary/20 bg-primary/5' },
    { label: 'Avg System Latency', value: `${avgSystemLatencySec}s`, desc: 'Average parse + analysis duration', icon: Clock, color: 'text-emerald-400 border-emerald-500/20 bg-emerald-500/5' },
    ...
    { label: 'Cumulative Tokens', value: combinedTokens.toLocaleString(), desc: 'Gemini usage index', icon: Brain, color: 'text-pink-400 border-pink-500/20 bg-pink-500/5' },
    { label: 'Parsed Resumes', value: totalResumes, desc: `${parserFailures} Failures logged`, icon: FileText, color: 'text-indigo-400 border-indigo-500/20 bg-indigo-500/5' },
    { label: 'Avg Job Fit', value: `${avgMatchScore}%`, desc: 'Average fit overlap score', icon: TrendingUp, color: 'text-teal-400 border-teal-500/20 bg-teal-500/5' },
  ];
    ...
    <div className="space-y-6 select-none animate-fade-in text-white">
      ...
      <div>
        <h2 className="text-xl sm:text-2xl font-bold font-display text-white tracking-tight">
          AI Career Analytics CMS
        </h2>
        <p className="text-xs text-text-muted mt-1">
          Monitor aggregate Career scores, recommended roadmap trends, system latency overheads, and token spending indexes.
        </p>
      </div>
      ...
      <div className="bg-zinc-950 border border-zinc-850 p-5 rounded-xl flex items-start gap-3">
        ...
        <div className="text-xs space-y-1 text-zinc-450">
          ...
          <p className="leading-relaxed">
            AI overhead costs are aggregated across both the Resume Parsing and Career Intelligence engines. Failure triggers are monitored via the Trigger.dev dashboard workspace. The database maintains full version control over snapshots to optimize data footprints.
          </p>
        </div>
      </div>
