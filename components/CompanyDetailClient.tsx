...
export default function CompanyDetailClient({
  ...
  }, [opportunities, search, selectedType, selectedRemote, sortBy]);
    ...
    <div className="max-w-7xl mx-auto space-y-6 animate-fade-in text-foreground pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 bg-surface-muted/40 p-3 rounded-xl border border-border-subtle">
        <div className="flex flex-wrap items-center gap-2.5 flex-1 min-w-0">
          {/* Search Box */}
          <div className="relative flex items-center gap-2 bg-input-bg border border-border-subtle rounded-lg px-3 py-1.5 w-full sm:w-72 focus-within:border-primary/50 transition-colors h-9">
            <Search className="w-3.5 h-3.5 text-text-muted shrink-0" />
            <input
              type="text"
              placeholder={`Search ${company.name} openings, skills...`}
              value={search}
      ...
      {filteredOpportunities.length > 0 ? (
            ...
            >
              <div className="space-y-2 flex-1 min-w-0">
                ...
                <div className="flex flex-wrap items-center gap-y-1 gap-x-4 text-xs text-text-muted font-medium">
                  <span className="inline-flex items-center gap-1">
                    <MapPin className="w-3.5 h-3.5 text-text-muted/70" />
                    {opp.location || opp.remoteType || 'Multiple locations'}
                  </span>
        ...
        <div className="border border-dashed border-border-subtle rounded-2xl p-12 text-center max-w-lg mx-auto space-y-3">
          ...
          <h3 className="text-base font-bold text-foreground">No matching openings found</h3>
          ...
          {company.careerPage && (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-1.5 text-xs text-primary hover:underline font-bold mt-2"
            >
