...
export default function SystemError({ error, reset }: ErrorProps) {
  useEffect(() => {
    console.error('System page route error boundary:', error);
  }, [error]);
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[400px] border border-zinc-800 bg-[#111113] rounded-xl p-8 text-center">
      ...
      <p className="text-xs text-text-muted max-w-sm mb-6 leading-relaxed">
        An error occurred while monitoring server state or testing connection pools. Try refreshing the page.
      </p>
