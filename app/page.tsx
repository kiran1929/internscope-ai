  ...
  return (
    <div className="preserve-dark-theme flex-1 flex flex-col min-h-screen text-foreground relative overflow-hidden bg-[#09090B]">
      ...
      <div className="flex-1 flex flex-col relative z-10">
        ...
        {/* TARGET COMPANIES SCROLLER - Infinite Freeform Marquee */}
        <motion.section
          initial={{ opacity: 0 }}
        ...
        >
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center space-y-10">
            ...
            {/* Marquee Wrapper */}
            <div className="relative flex overflow-x-hidden mask-horizontal py-2">
              <motion.div
              ...
              >
                {[...INITIAL_COMPANIES, ...INITIAL_COMPANIES].map((company, index) => (
                  <div
                    key={`${company.id}-${index}`}
                    className="flex items-center gap-3 px-4 py-2.5 rounded-2xl bg-zinc-900/30 hover:bg-zinc-850/60 backdrop-blur-xs transition-all duration-300 group cursor-pointer hover:scale-105 hover:-translate-y-0.5"
                    title={company.name}
                    onClick={handleNavigateToDashboard}
                  >
                    <CompanyLogo logo={company.logo} name={company.name} size="sm" />
                    <span className="text-xs font-semibold text-zinc-300 group-hover:text-white font-sans tracking-tight whitespace-nowrap">
                      {company.name}
                    </span>
                  </div>
                ))}
