...
export default function NotificationsError({ error, reset }: ErrorProps) {
  useEffect(() => {
    console.error('Notifications page route error boundary:', error);
  }, [error]);
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[400px] border border-zinc-800 bg-[#111113] rounded-xl p-8 text-center">
      ...
      <p className="text-xs text-text-muted max-w-sm mb-6 leading-relaxed">
        An error occurred while loading notifications or updating their read/unread states. Try refreshing the page.
      </p>
